...
-(void) generateNewSecretAndPublicKey {
    ...
    TECPoint *basePt = [[TECPoint alloc] initEd25519BasePointWithoutCurve];
    publicKey = [TECPoint addEd25519: basePt kTimes: secretKeyFGInt];
    [secretKeyFGInt eraseAndRelease];
    [basePt release];

    result = SecRandomCopyBytes(kSecRandomDefault, [tmpData length], [tmpData mutableBytes]);
    [tmpData release];
}


-(NSData *) secretKeyToNSData {
    return [secretKey copy];
}
-(NSData *) publicKeyToNSData {
    return [publicKey to25519NSData];
}
-(NSString *) secretKeyToBase64NSString {
    return [FGInt convertNSDataToBase64: secretKey];
}
-(NSString *) publicKeyToBase64NSString {
    return [FGInt convertNSDataToBase64: [self publicKeyToNSData]];
}
-(void) setSecretKeyWithNSData: (NSData *) secretKeyNSData {
    if ([secretKeyNSData length] != 32) {
        NSLog(@"secretKeyNSData is corrupt for %s at line %d. Expected 32 bytes but received %lu", __PRETTY_FUNCTION__, __LINE__, [secretKeyNSData length]);
    }
    secretKey = [secretKeyNSData copy];
}
-(void) setSecretKeyWithBase64NSString: (NSString *) secretKeyBase64NSString {
...
-(void) setPublicKeyWithNSData: (NSData *) publicKeyNSData {
    ...
    publicKey = [[TECPoint alloc] initFromCompressed25519NSDataWithoutCurve: publicKeyNSData];
}
-(void) setPublicKeyWithBase64NSString: (NSString *) publicKeyBase64NSString {
...
-(NSData *) signNSData: (NSData *) plainText {
    ...
    [tmpData release];
    TECPoint *basePt = [[TECPoint alloc] initEd25519BasePointWithoutCurve];
    TECPoint *rPoint = [TECPoint addEd25519: basePt kTimes: rFGInt];
...
-(BOOL) verifySignature: (NSData *) signatureData ofPlainTextNSData: (NSData *) plainText {
    ...
    TECPoint *basePt = [[TECPoint alloc] initEd25519BasePointWithoutCurve];
    TECPoint *verificationPoint = [TECPoint addEd25519: basePt k1Times: sFGInt and: publicKey k2Times: hashFGInt];
    [basePt release];
    [sFGInt release];
    [hashFGInt release];
